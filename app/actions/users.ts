...
export async function createAdminUser(input: unknown) {
  ...
  if (!parsed.success) {
    return { ok: false as const, error: "Please check the name, email, and password." };
  }
  ...
  await db.insert(admins).values({
    email,
    passwordHash,
    name: sanitizeName(parsed.data.name) || "Host",
    isOwner: false,
    isActive: true,
  });
...
export async function setAdminAccess(id: string, isActive: boolean) {
  const { admin: me } = await requireOwner();
  if (id === me.id) {
    return { ok: false as const, error: "You can't change your own access." };
  }
  ...
  if (target.isOwner) {
    return { ok: false as const, error: "The owner account can't be disabled." };
  }
